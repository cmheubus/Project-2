...
```{r Calculating Summaries of Each Variable, echo=false}
...
Though not covered in the class notes, I decided to experiment with a slightly different approach. Working off the model that fits all main effect, I tried to reduce the number of variables using **backwards selection** - starting with all variables, and then narrowing them down based upon p-value.
```{r}
...
#### Comparing Root MSE 
While the fit statistics are one way to determine which model is best, **prediction error** is actually preferred. After looking at several options, I decided to use cross-validation, per the HW 9 prediction key from Ari. With this approach, which uses 10 folds, I was able to calculate the Root Mean Square Error for the two models on the Training set.  
